use chrono::{DateTime, Duration, Local, NaiveDate, TimeZone, Datelike, Weekday, Timelike};
...
impl CommitStats {
    pub fn formatted_lines(&self, desc: bool) -> Vec<String> {
        ...
        let mut v: Vec<_> = self.data.iter().map(|(email, m)| {
            (m.count, format!(
                "{:<30} {:>4} commits   🗓  {} → {}",
                email, m.count, fmt_date(m.first), fmt_date(m.last)
            ))
        }).collect();
...
pub fn collect_commits(
    ...
    for id in rw.flatten() {
        ...
        if let Some(min) = since { if dt < min { continue; } }

        seen += 1;
        let e = data.entry(email).or_insert(AuthorMeta { count:0, first:dt, last:dt });
        e.count += 1;
        if dt < e.first { e.first = dt; }
        if dt > e.last  { e.last  = dt; }
    }
    CommitStats { total_seen: seen, data }
}
...
pub fn scan_repo(repo: &Repository, limit: Option<usize>) -> RepoScan {
    ...
    for id in rw.flatten() {
        if let Some(max) = limit { if seen >= max { break; } }
        ...
        // per-author stats
        seen += 1;
        let e = data.entry(email.clone()).or_insert(AuthorMeta { count:0, first:dt, last:dt });
        e.count += 1;
        if dt < e.first { e.first = dt; }
        if dt > e.last  { e.last  = dt; }
        ...
        *day_counts.entry(day).or_default() += 1;
        ...
        match first_date {
            None => { first_date = Some(dt); first_author = email.clone(); }
            Some(d) => if dt < d { first_date = Some(dt); first_author = email.clone(); }
        }
        match last_date {
            None => { last_date = Some(dt); last_author = email.clone(); }
            Some(d) => if dt > d { last_date = Some(dt); last_author = email.clone(); }
        }
        ...
        if (9..=17).contains(&hour) { work_hours_hits += 1; }

        if commit.parent_count() > 1 { merges += 1; }
    ...
    let (first_date, last_date) = match (first_date, last_date) {
        (Some(f), Some(l)) => (f, l),
        _ => { let now = Local::now(); (now, now) }
    };

    // Active period & average
    let active_days = (last_date.date_naive() - first_date.date_naive()).num_days().max(0) + 1;
    let avg_commits_per_day = if active_days > 0 {
        total_commits as f64 / active_days as f64
    } else { 0.0 };

    // Peak day
    let peak_day = day_counts.iter().max_by_key(|(_, c)| **c)
        .map(|(d, c)| (*d, *c));
    ...
    for w in all_dates.windows(2) {
        if let [a, b] = w {
            ...
            if gap > longest_idle_gap_days { longest_idle_gap_days = gap; }
        }
    }
    ...
    // Merge/Revert rates
    let merge_rate  = if total_commits > 0 { (merges as f64) * 100.0 / total_commits as f64 } else { 0.0 };
    let revert_rate = if total_commits > 0 { (reverts as f64) * 100.0 / total_commits as f64 } else { 0.0 };
    ...
    for (email, dt) in &commits_log {
        if *dt >= t90 { active_last_90.insert(email.clone()); commits_last_90 += 1; }
        if *dt >= t30 { *recent30_counts.entry(email.clone()).or_default() += 1; }
    }
    ...
    let mut counts: Vec<(String, usize)> =
        data.iter().map(|(k, v)| (k.clone(), v.count)).collect();
    ...
    if total_c == 0 { core_size_80pct = 0; }
    ...
    RepoScan {
        stats: CommitStats { total_seen: total_commits, data },
        summary,
    }
}
